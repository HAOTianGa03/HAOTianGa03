# Hi there, I'm GAOTIANHAO 👋

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=200&section=header&text=Welcome%20to%20My%20Profile!&fontSize=50&animation=fadeIn" width="100%" />
</div>

🎓 **深圳大学 (SZU) 2022级本科生** | 🌟 **电子信息工程专业**

我是一名对具身智能和机器人控制充满热情的开发者与研究者。目前，我正专注于**灵巧手操作算法**的研发与探索。

### 🔬 研究方向与兴趣 (Research & Interests)
- 🤖 **主要方向:** 视觉-语言-动作模型 (VLA, Vision-Language-Action) & 强化学习操作 (RL Manipulation)
- 🖐️ **当前工作:** 灵巧手 (Dextrous Hand) 复杂操作算法的设计与实现

### 🛠️ 技术栈 (Tech Stack)

**编程语言 (Languages):**
<p>
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white" alt="C++"/>
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python"/>
  <img src="https://img.shields.io/badge/Shell_Script-121011?style=for-the-badge&logo=gnu-bash&logoColor=white" alt="Shell"/>
</p>

**专业领域 (Domains):**
- 🧱 **嵌入式软件 (Embedded Software):** 硬件底层驱动、RTOS 及软硬协同设计
- ⚡ **AI Infra:** 模型训练框架与底层加速部署
- 🧠 **具身智能算法 (Embodied AI):** 强化学习、模仿学习以及基于 VLA 的机器人控制
- 🔌 **机器人中间件 (Robot Middleware):** 熟悉 ROS / ROS 2 等机器人通信与控制生态

### 📊 GitHub 统计 (Stats)

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=HAOTianGa03&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117" alt="HAOTianGa03's GitHub Stats" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=HAOTianGa03&theme=tokyonight&hide_border=true&background=0D1117" alt="HAOTianGa03's GitHub Streak" />
</div>

---
*If you are interested in my work, feel free to reach out!* 🚀
